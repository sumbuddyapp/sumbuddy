...
import { getPostData, getCompanyData } from "@/lib/fetchers";
...
export async function generateMetadata({
  ...
  return {
    ...
    //     alternates: {
    //       canonical: `https://${companyData.customDomain}/${params.slug}`,
    //     },
    //   }),
  };
}
...
export async function generateStaticParams() {
  ...
  const allPaths = allPosts
    .flatMap(({ company, slug }) => [
      company?.subdomain && {
        domain: `${company.subdomain}.${process.env.NEXT_PUBLIC_ROOT_DOMAIN}`,
        slug,
      },
        ...
        <a
          ...
          href={
            data.company?.user?.username
              ? `https://twitter.com/${data.company.user.username}`
              : `https://github.com/${data.company?.user?.gh_username}`
          }
          rel="noreferrer"
          target="_blank"
        >
          <div className="my-8">
            <div className="relative inline-block h-8 w-8 overflow-hidden rounded-full align-middle md:h-12 md:w-12">
              {data.company?.user?.image ? (
                <BlurImage
                  alt={data.company?.user?.name ?? "User Avatar"}
                  height={80}
                  src={data.company.user.image}
                  width={80}
                />
              ) : (
            ...
            <div className="text-md ml-3 inline-block align-middle dark:text-white md:text-lg">
              by <span className="font-semibold">{data.company?.user?.name}</span>
            </div>
          </div>
        </a>
      </div>

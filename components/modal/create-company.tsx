  ...
  return (
    <form
      action={async (data: FormData) =>
        createCompany(data).then((res: any) => {
          if (res.error) {
            toast.error(res.error);
          } else {
            va.track("Created Company");
            const { id } = res;
            router.refresh();
    ...
    >
      <div className="relative flex flex-col space-y-4 p-5 md:p-10">
        <h2 className="font-cal text-2xl dark:text-white">Create a new Company</h2>
        ...
        <div className="flex flex-col space-y-2">
          ...
          <textarea
            name="description"
            placeholder="Description about why my Company is so awesome"
      ...
      <div className="flex items-center justify-end rounded-b-lg border-t border-stone-200 bg-stone-50 p-3 dark:border-stone-700 dark:bg-stone-800 md:px-10">
        <CreateCompanyFormButton />
      </div>
    </form>
  );
}
function CreateCompanyFormButton() {
  ...
  return (
    <button
      ...
      disabled={pending}
    >
      {pending ? <LoadingDots color="#808080" /> : <p>Create Company</p>}
    </button>
  );
}

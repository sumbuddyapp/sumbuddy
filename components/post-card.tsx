...
import { Post, Company } from "@prisma/client";
...
export default function PostCard({
  ...
  data: Post & { company: Company | null };
}) {
  const url = `${data.company?.subdomain}.${process.env.NEXT_PUBLIC_ROOT_DOMAIN}/${data.slug}`;
        ...
        <a
          href={
            process.env.NEXT_PUBLIC_VERCEL_ENV
              ? `https://${url}`
              : `http://${data.company?.subdomain}.localhost:3000/${data.slug}`
          }
          target="_blank"
          rel="noreferrer"
